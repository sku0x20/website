...
**Senior Systems Architect · Staff Systems Engineer**  
...
* **ClickHouse Ingestion Engine (95%+ Compression):** Resolved a filesystem inode exhaustion crisis caused by nested directory log dumps. Designed high-throughput columnar ClickHouse schemas (`FixedString`, `LowCardinality`, Delta + ZSTD codecs, sparse composite primary keys), slashing telemetry footprint by 95%+ (compressing `device_health` telemetry from 17 GB down to 300 MB on disk).
...
* **Binary Payload Decoupling:** Architected and executed a zero-downtime data migration pipeline extracting legacy binary image blobs out of primary database tables, streaming them to secure Google Cloud Storage (GCS) buckets, and swapping schemas to immutable URIs.
